...
class TrackState:
    ...
    has been collected. Then, the track state is changed to `confirmed`. Tracks
    that are no longer alive are classified as `deleted` to mark them for
...
class Track:
    ...
    def delete(self):
        """Mark this track as deleted (no association at the current time
        step)."""
        self.state = TrackState.Deleted

    def revive(self):
        """Mark this track as alive (may become confirmed)."""
        self.state = TrackState.Tentative

    def re_id(self, id):
        """Re-assign a track to a new id."""
        self.track_id = id
